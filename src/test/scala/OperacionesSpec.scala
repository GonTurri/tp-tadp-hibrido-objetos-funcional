...
class OperacionesSpec extends AnyFunSpec with Matchers {
  ...
  describe("opt combinator") {
    ...
    it("con default") {
      val resulto = parserStringIn.optConDefault("Jorge")
      val aplicado1 = resulto("infija")
      val result2 = resulto("fija")

      aplicado1 shouldBe Success(Resultado("in", "fija"))
      result2 shouldBe Success(Resultado("Jorge","fija"))
    }
